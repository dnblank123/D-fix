...
void STDMETHODCALLTYPE ID3D11DeviceContext_UpdateSubresource(
    ...
    if (SUCCEEDED(pDstResource->QueryInterface(__uuidof(ID3D11Buffer), (void**)&buf))) {
        D3D11_BUFFER_DESC desc;
        buf->GetDesc(&desc);  // :contentReference[oaicite:8]{index=8}

        // 2) If this is *your* dynamic CB, perform Map/Unmap instead:
        if (desc.Usage == D3D11_USAGE_DYNAMIC &&
            (desc.BindFlags & D3D11_BIND_CONSTANT_BUFFER))
        {
            D3D11_MAPPED_SUBRESOURCE mapped{};
            // Discard previous contents for best performance :contentReference[oaicite:9]{index=9}
            if (SUCCEEDED(pContext->Map(buf, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped))) {
                // Copy new data
                memcpy(mapped.pData, pSrcData, desc.ByteWidth);
                pContext->Unmap(buf, 0);
            }
            buf->Release();
            return;   // Skip original UpdateSubresource
        }
        buf->Release();
    }
